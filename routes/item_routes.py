    ...
    try:
        ...
        try:
            if experts_collection is not None:
                # Fetch item details for email subject and PDF
                item = items_collection.find_one({'_id': ObjectId(item_id)})
                item_title = item.get('title', 'Interview Board') if item else 'Interview Board'
                item_no = item.get('itemNo') if item else None
                discipline = item.get('discipline', item_title) if item else item_title
                
                # Fetch advertisement number
                adv_no = None
                if item and item.get('advertisementId') and advertisements_collection is not None:
                    adv = advertisements_collection.find_one({'_id': item['advertisementId']})
                    if adv:
                        adv_no = adv.get('advertisementNo')
